...
DATASETS = {}
DATASETS['VCTK-Corpus'] = {"FILE_PATTERN": r'p([0-9]+)_([0-9]+)\.wav', "TRAINING_RANGE": 300, "id_index": 0, "num_index": 1}
DATASETS['CMU-Arctic-SLT'] = {"FILE_PATTERN": r'arctic_a([0-9]+)\.wav', "TRAINING_RANGE": 1000, "id_index": None, "num_index": 0}
...
TRAINING_RANGE = 300
# CMU Arctic SLT
# FILE_PATTERN = r'arctic_a([0-9]+)\.wav'
# TRAINING_RANGE = 1000

def get_category_cardinality(files, dataset):
    id_reg_exp = re.compile(DATASETS[dataset]['FILE_PATTERN'])
    min_id = None
    max_id = None
    for filename in files:
        matches = id_reg_exp.findall(filename)[0]
        id = matches[DATASETS[dataset]['id_index']]
...
def align_local_condition(local_condition, length, factor=None):
    # TODO
    if factor is None:
        factor = int(length / (local_condition.shape[0]-1))
    
    upsampled_lc = ops.upsample_center_fill(factor, local_condition)
    upsampled_lc = upsampled_lc[:length]
    
    diff = length - upsampled_lc.shape[0]
    if diff>0:
        upsampled_lc = np.pad(upsampled_lc, [[0, diff], [0, 0]],
                          'constant')
    
    # print(str(length) +" "+str(local_condition.shape[0])+" Pad: "+str(diff)+" Factor: "+str(factor))
    return upsampled_lc


def load_generic_audio(directory, sample_rate, lc_maps, dataset):
    ...
    for filename in randomized_files:
        ...
        if (not ids) or (not DATASETS[dataset]['id_index']):
            # The file name does not match the pattern containing ids, so
            # there is no id.
            category_id = None
        else:
            # The file name matches the pattern for containing ids.
            category_id = int(ids[0][DATASETS[dataset]['id_index']])
            if int(ids[0][DATASETS[dataset]['num_index']]) >= DATASETS[dataset]['TRAINING_RANGE']:
                continue
        # print(filename)
        ...
        if lc_maps:
            ...
            if "factor" in lc_maps:
                lc = align_local_condition(lc, audio.shape[0], int(lc_maps["factor"]))
            else:
                lc = align_local_condition(lc, audio.shape[0], None)
        else:
            lc = None
...
def not_all_have_id(files, dataset):
...
class AudioReader(object):
    ...
    def __init__(self,
        ...
        if dataset not in DATASETS:
            raise ValueError("Provided dataset is not in record.")
        else:
        ...
        if lc_maps_json is not None:
            ...
            """For test set"""
            self.test_lc_placeholder = tf.placeholder(dtype=tf.float32, shape=None)
            self.test_lc_queue = tf.PaddingFIFOQueue(queue_size,
                                         ['float32'],
                                         shapes=[(None, None)])
            self.test_lc_enqueue = self.test_lc_queue.enqueue([self.test_lc_placeholder])
        ...
        if self.gc_enabled and not_all_have_id(files, self.dataset):
            raise ValueError("Global conditioning is enabled, but file names "
                             "do not conform to pattern having id.")
        # Determine the number of mutually-exclusive categories we will
        # accomodate in our embedding table.
        if self.gc_enabled:
            _, self.gc_category_cardinality = get_category_cardinality(files, self.dataset)
    ...
    def dequeue_test_lc(self, num_elements):
        return self.test_lc_queue.dequeue_many(num_elements)
...
#                         if self.lc_maps is not None:
#                             lc_piece = lc[:(self.receptive_field + self.sample_size), :]

#                             """For test set"""
#                             sess.run(self.test_lc_enqueue,
#                                      feed_dict={self.test_lc_placeholder: lc_piece})
                            
#                             lc = lc[self.sample_size:, :]
#                 else:
                    """For test set"""
                sess.run(self.test_enqueue,
                             feed_dict={self.test_placeholder: audio[:15000]})

                if self.lc_maps is not None:
                    """For test set"""
                    sess.run(self.test_lc_enqueue,
                                 feed_dict={self.test_lc_placeholder: lc[:15000, :]})
